...
function activate(context) {
    ...
    let disposable = vscode.commands.registerCommand('extension.generateUnitTests', async () => {
        ...
        const backendUrl = configuration.get('backendUrl');
        const testFramework = configuration.get('testFramework');
        const testExecutionCommand = configuration.get('testExecutionCommand');
...
function createOrShowWebview(extensionUri) {
    ...
    const panel = vscode.window.createWebviewPanel(panelId, 'IntelliTesting AI', column, {
        enableScripts: true,
